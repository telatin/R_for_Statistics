...
5. (Difficult) How many `viriginca` flowers are included in the dataset?
...
:::{callout-note}
Although this `boxplot` function isn't very useful, note the syntax 
:::
...
Then use either the base function `confint` or the `tidy` function from the `broom` package (`broom` is installed with `tidyverse`) to get the confidence intervals for those differences:
...
library(broom)
model2 |> augment(interval="confidence")
```

Finally we can use ggplot to show these predicted values and confidence intervals:
